        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        @if (!empty($application->loan->branch->bank->bank_logo))
                            <div class="text-center mb-3">
                                <img src="{{ asset($application->loan->branch->bank->bank_logo) }}"
                                    alt="{{ $application->loan->branch->bank->bank_name ?? $application->loan->branch->bank->name }}"
                                    class="img-fluid" style="max-height: 60px;">
                            </div>
                        @endif

                        <h6 class="fw-bold mb-1">{{ $application->loan->name ?? 'N/A' }}</h6>
                        <p class="text-muted small mb-3">
                            {{ $application->loan->branch->bank->bank_name ?? ($application->loan->branch->bank->name ?? '') }}
                        </p>

                        <div class="mb-2">
                            <small class="text-muted">Branch</small>
                            <p class="mb-0"><strong>{{ $application->loan->branch->name ?? '' }}</strong></p>
                        </div>

                        <div class="mb-2">
                            <small class="text-muted">Interest Rate</small>
                            <p class="mb-0"><strong>{{ $application->loan->interest_rate ?? 'N/A' }}% per annum</strong>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body">
                        <form action="{{ route('bank-admin.applications.updateStatus', $application) }}" method="POST">
                            @csrf
