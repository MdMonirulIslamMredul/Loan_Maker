...
@section('dashboard-title', 'Branch Admin - Loan Management')

@section('content')
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="mb-0 fw-bold">Loans for {{ Auth::user()->branch->name }}</h2>
            <a href="{{ route('branch-admin.loans.create') }}" class="btn btn-primary">
                            ...
                            <tr>
                                ...
                                <td class="small">
                                    @if($loan->min_tenure_months || $loan->max_tenure_months)
                                        {{ $loan->min_tenure_months ?? 'N/A' }} - {{ $loan->max_tenure_months ?? 'N/A' }} months
                                    @else
                                        N/A
                                    @endif
                                </td>
                                ...
                                <td>
                                    <a href="{{ route('branch-admin.loans.edit', $loan) }}" class="btn btn-sm btn-outline-primary me-1">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{ route('branch-admin.loans.destroy', $loan) }}"
                                          method="POST"
                                          class="d-inline"
